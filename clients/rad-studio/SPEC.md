...
| Semantic tokens (type names) | **Have** (0.48.0, 2026-09-22) | `PaintText` AFTER-event: the type sub-spans of a run repainted over the IDE's text (`PasTreeIdePlugin.SemanticPaint`) | Spike findings, all three routes paint: a run is coalesced by paint attribute and coded by its first non-blank cell (`(TWinControl);` is one atSymbol run), so a type name is a sub-span located by column, pixels proportional within the run rect; `Canvas.Font.Color` set in the before-event IS honoured (whole run); `AllowDefaultPainting := False` + own paint works too. After-event overpaint chosen: the IDE keeps styles, selection and the current-line band. Tokens cached per file by the session (`semanticTokens/full`), refreshed on each `publishDiagnostics` for a viewed file, the previous answer kept between edits. `IOTAHighlighter` (`1801`) remains the wrong route: synchronous per line, no new colour registrable (`INTACodeEditorOptions` `881` read-only) |
